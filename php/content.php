<?php
//Including the session.php file so that we can take the username of the user and display it
include('session.php'); 
//If user is not logged in, it takes them back to index.php
//Used this so user cannot simply type in the name of the page and access it.
if(!isset($_SESSION['userLogin'])){ 
  header("location: index.php"); 
}
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="BN Sports DB allows you to look up and obtain information about professional athletes!">
    <!-- Linking all of my stylesheets -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link rel="stylesheet" href="../css/content.css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.1/css/all.css" integrity="sha384-50oBUHEmvpQ+1lW4y57PTFmhCaXp0ML5d60M1M7uH2+nqUivzIebhndOJK28anvf" crossorigin="anonymous">
    <link rel="icon" 
      type="image/png" 
      href="../images/favicon.png">
    <!-- Title of page -->
    <title>BN Sports DB</title>
  </head>
  <!-- Start of body -->
  <body>
    <header>
        <!-- Start of navbar -->
        <nav>
            <ul>
                <a href="../index.html"><li class="left">BN Sports DB<i class="fas fa-trophy"></i></li></a> 
                <a href="../about.html"><li class="right">About</li></a>
                <!-- If user clicks on the logout item, they are then logged out and taken back to index.php -->
                <a href="logout.php"><li class="right">Log Out</li></a>
            </ul>          
        </nav>
        <!-- Start of section -->
        <section>
            <div id="information">
                <!-- Displays username of logged in user -->
                <h1>Welcome, <?php echo $loginUsername; ?>!</h1>
                <p>Search for any professional sports player!</p>
                <input id="search" type="text" placeholder="Search for players here..." /><br><br>
                <a href="#results"><button id="searchBtn">Search</button></a>
            </div>
            <!-- Footer with all the social media icons -->
            <footer>
                    <ul>
                      <li><i class="fab fa-facebook"></i></li>
                      <li><i class="fab fa-instagram"></i></li>
                      <li><i class="fab fa-twitter"></i></li>
                      <li><i class="fab fa-snapchat"></i></li>
                      <li><i class="fab fa-youtube"></i></li>
                    </ul>
            </footer>
        </section>
    </header>
    <!-- End start of page -->
        
        <!-- Boilerplate that will store all of the information we recieve from JSON-->
        <div id="results">
            <div class="container">
                <div class="row">
                </div>
            </div>
        </div>

    <!-- Linking all my JavaScript to the page -->
    <script src="https://code.jquery.com/jquery-3.4.0.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
    <script src="../js/content.js"></script>
    </body>
</html>